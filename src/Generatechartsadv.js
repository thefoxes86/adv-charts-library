...
globalThis.Generatechartsadv = /** @class */ (function () {
    ...
    Generatechartsadv.prototype.iterateMainElement = function (cards) {
        ...
        cards.forEach(function (el, index) {
            ...
            var previousPercValue = document.getElementById("mainValuePerc-".concat(_this.arrayAdv[index].type, "-").concat(index));
            var value = "".concat(_this.arrayAdv[index].realValues[_this.arrayAdv[index].realValues.length - 1]).concat(_this.arrayAdv[index].valueLabels[_this.arrayAdv[index].valueLabels.length - 1]);
    ...
    Generatechartsadv.prototype.iterateRow = function (contentRow, index) {
        ...
        contentRow.forEach(function (row, indexRow) {
            ...
            var previousValue = document.getElementById("perc-value-".concat(_this.arrayAdv[index].type, "-").concat(index, "-row-").concat(indexRow));
            if ((previousValue === null || previousValue === void 0 ? void 0 : previousValue.innerHTML) !== "".concat(_this.arrayAdv[index].realValues[indexRow]).concat(_this.arrayAdv[index].valueLabels[indexRow])) {
