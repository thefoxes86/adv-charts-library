...
window.addEventListener("DOMContentLoaded", ()=>{
    const chartAdvert = new (0, _generatechartsadvDefault.default)((0, _advertDefault.default), "chart-card");
    const chartHome = new (0, _generatechartsadvDefault.default)((0, _homeDefault.default), "chart-card");
    const chartGmb = new (0, _generatechartsadvDefault.default)((0, _gmbDefault.default), "chart-card");
    setTimeout(()=>{
        chartAdvert.init();
    }, 1000);
    setTimeout(()=>{
...
},{"./api/advert":"axN5u","./api/home":"jQFBD","./api/gmb":"aB3WZ","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3","./Generatechartsadv":"7WdTZ"}],"axN5u":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
...
},{"@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"aB3WZ":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
const gmb = [
    {
...
class Generatechartsadv {
    constructor(array, className){
        ...
        this.typedCard = this.chartCard.filter((elem)=>elem.dataset["type"] === array[0].type);
    }
    init() {
    ...
    iterateMainElement(cards) {
        cards.forEach((el, index)=>{
            ...
            this.header.classList.add(this.arrayAdv[index].valueColors[this.arrayAdv[index].valueColors.length - 1]);
            this.circle.style["stroke-dasharray"] = `${this.arrayAdv[index].percValues[this.arrayAdv[index].percValues.length - 1]} 100`;
            this.percValue = new (0, _countupJs.CountUp)(`mainValuePerc-${this.arrayAdv[index].type}-${index}`, this.arrayAdv[index].realValues[this.arrayAdv[index].realValues.length - 1], {
                suffix: this.arrayAdv[index].valueLabels[this.arrayAdv[index].valueLabels.length - 1]
            });
    ...
    iterateRow(contentRow, index) {
        contentRow.forEach((row, indexRow)=>{
            ...
            this.percValueText = new (0, _countupJs.CountUp)(`perc-value-${this.arrayAdv[index].type}-${index}-row-${indexRow}`, this.arrayAdv[index].realValues[indexRow], {
                suffix: this.arrayAdv[index].valueLabels[indexRow]
            });
            this.percValueText.start();
            this.textValue = document.getElementById(`perc-value-${this.arrayAdv[index].type}-${index}-row-${indexRow}`);
